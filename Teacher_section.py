...
def Lsave_details():
    ...
    cursor.execute("INSERT INTO teacher (first_name, last_name, subject_name, email,Lec_Type,password) VALUES (?, ?, ?, ?, ?,'teacher123')", (f.rstrip(), l.rstrip(), s.rstrip()+" Lab", e.rstrip(),"lab"))
    cursor.execute("SELECT * FROM teacher where Lec_Type='lab'")
    print(cursor.fetchall())
    conn.commit()
