...
def fill_random_tt():
    ...
    lab_subjects = set()  # Set to keep track of lab subjects for each day
    ...
    for day in days:
        ...
        # Reset lab count and subjects for each day
        lab_count_per_day = 0  
        lab_subjects.clear()
        ...
        for slot in time_slots:
            if slot != "BREAK":
                ...
                # Handling labs
                if slot.startswith("1:30") and lab_count_per_day < 3:
                    lab_subject = "Lab" + str(lab_count_per_day + 1)
                    room = "Lab" + str(lab_count_per_day + 1)
                    lab_count_per_day += 1

                    # Ensure the lab subject is unique for the day
                    while lab_subject in lab_subjects:
                        lab_subject = "Lab" + str(lab_count_per_day + 1)
                        room = "Lab" + str(lab_count_per_day + 1)

                    lab_subjects.add(lab_subject)

                    # Assign lab sessions to batches A, B, and C
                    for batch in ['A', 'B', 'C']:
                        lab_teacher = formatted_data_A.pop(0)
                        data = [lab_subject + ' - Batch ' + batch, lab_teacher[0], room, day, slot]
                        save_data.append(data)

                # Handling regular lectures
                else:
                    if formatted_data_A:  # Ensure there are teachers left
                        if formatted_data_A[0][3] < 3 and non_free_lecture_count_per_day < 4:
                            data = [formatted_data_A[0][1], formatted_data_A[0][0], formatted_data_A[0][2], day, slot]
                        ...
                        else:  # If no more non-free lectures available, add a free lecture
                            if free_lecture_count_per_day < 3 or non_free_lecture_count_per_day > 3:
                                data = ["Free", "", "", day, slot]
    ...
    if len(formatted_data_A) < 5:
        msg = messagebox.showerror("ERROR", "You must have at least 5 teachers to generate a valid TT")
        return ValueError
    ...
    for item in save_data:
        cursor.execute("INSERT INTO timetable (subject_name, teacher_name, room_number, day, time_slot) VALUES (?, ?, ?, ?, ?)", item)
        conn.commit()
...
"""ye wala vo back to back free lecture wala hai """
# def fill_random_tt():
#     save_data.clear()
#     show_focusA(event=None)
